...
public class Classroom {
    ...
    public void enterClassroom ( String filename ) {
        ...
        for(int i = 0; i < numberOfStudents; i++){
            firstNameOfStudent = StdIn.readString();
            lastNameOfStudent = StdIn.readString();
            studentHeight = StdIn.readInt();

            studentList[i] = new Student(firstNameOfStudent, lastNameOfStudent, studentHeight);
        }
        Student tmpr;
        for(int i = 0; i < numberOfStudents; i++){
            for(int j = 0; j < studentList.length - 1; j++){
                if(studentList[j].compareNameTo(studentList[j + 1]) > 0){
                    tmpr = studentList[j];
                    studentList[j] = studentList[j + 1];
                    studentList[j + 1] = tmpr;
                }
            }
        }
        ...
        for(int i = 0; i < studentList.length; i++){
            pointer.setStudent(studentList[i]);
            if(i < studentList.length - 1){
                pointer.setNext(new SNode());
            }
    ...
    public void seatStudents () {
        ...
        int r = seatingLocation.length;

        int c = seatingLocation[0].length;


        if (musicalChairs != null) {
            for (int i = 0; i < r; i++) {
                for (int j = 0; j < c; j++) {
                    if (seatingLocation[i][j] && musicalChairs != null) {
    ...
    public void insertMusicalChairs () {
        ...
        for(int i = 0; i < studentsSitting.length && center > 0; i++){
            for(int j = 0; j < studentsSitting[i].length; j++){
                if(studentsSitting[i][j] != null) {
                    if(numberOfStudents == center){
                        SNode set = new SNode(studentsSitting[i][j], null);
                        musicalChairs = set;
                        pointer = set;
    ...
    public void insertByHeight(Student studentToInsert) {
        ...
        while (current.getNext() != null && studentToInsert.getHeight() > current.getNext().getStudent().getHeight()) {
            current = current.getNext();
        }
        
        SNode newNode = new SNode(studentToInsert, current.getNext());
    ...
    public void eliminateLosingStudents() {
        ...
        if(musicalChairs == null){
            ...
            s =1;
            SNode c = musicalChairs.getNext();
        
            while(c != musicalChairs ){
                s++;
                c = c.getNext();
            }
    ...
    public void seatMusicalChairsWinner () {
        ...
        int rws = studentsSitting.length;
        int clmn = studentsSitting[0].length;
